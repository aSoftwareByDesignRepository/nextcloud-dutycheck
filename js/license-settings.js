/**
 * DutyCheck — License settings panel (DTY2 mobile seats).
 *
 * Seats-only: DutyCheck has no room-display/signage product. The web app always stays
 * free; this script only manages the DTY2 license key and named mobile seats. It has no
...
(function () {
	...
	var root = document.getElementById('dc-license-panel');
	if (!root) {
		return;
	}
	...
	// ---- DOM references ---------------------------------------------------------------
	var liveEl = document.getElementById('dc-license-live');
	var alertLiveEl = document.getElementById('dc-license-alert');
	var feedbackEl = document.getElementById('dc-license-feedback');

	var badgeEl = document.getElementById('dc-license-badge');
	var validUntilEl = document.getElementById('dc-license-valid-until');
	var meterEl = document.getElementById('dc-license-meter');
	var meterFillEl = document.getElementById('dc-license-meter-fill');
	var meterTextEl = document.getElementById('dc-license-meter-text');
	var expiryCalloutEl = document.getElementById('dc-license-expiry-callout');
	var expiryBodyEl = document.getElementById('dc-license-expiry-body');

	var applyForm = document.getElementById('dc-license-form');
	var keyInput = document.getElementById('dc-license-key');
	var saveBtn = document.getElementById('dc-license-save');
	var removeBtn = document.getElementById('dc-license-remove');

	var seatSearchInput = document.getElementById('dc-license-seat-search-input');
	var seatSuggestEl = document.getElementById('dc-license-seat-search-suggest');
	var seatTbody = document.getElementById('dc-license-seat-tbody');

	var modalEl = document.getElementById('dc-license-confirm-modal');
	var modalCancelBtn = document.getElementById('dc-license-confirm-cancel');
	var modalConfirmBtn = document.getElementById('dc-license-confirm-ok');
	...
	// ---- Networking ---------------------------------------------------------------------
	function isNavOrAbort(resOrCause) {
		...
		if (window.DutyCheckApi && typeof window.DutyCheckApi.isAborted === 'function'
			&& window.DutyCheckApi.isAborted(resOrCause)) {
			return true;
		}
		if (resOrCause && resOrCause.aborted) {
			return true;
		}
		if (resOrCause && resOrCause.name === 'AbortError') {
			return true;
		}
		return false;
	}

	function showNetworkErrorUnlessAbort(resOrCause) {
		if (isNavOrAbort(resOrCause)) {
			return;
		}
	...
	function request(url, options) {
		if (!url) {
			return Promise.resolve({ ok: false, status: 0, data: null, aborted: false });
		}
		var opts = options || {};
			...
			.then(function (res) {
				return res
					...
					.then(function (raw) {
						...
						return { ok: res.ok, status: res.status, data: data, aborted: false };
					});
			})
	...
	function showFeedback(message, type) {
		...
		feedbackEl.className = 'dc-license-feedback' + (type ? ' dc-license-feedback--' + type : '');
	...
	function renderStatus(status) {
		...
		if (badgeEl) {
			...
			badgeEl.className = 'dc-license-badge ' + badgeClass;
		}
		...
		// seatsUsedText uses literal {used}/{total} placeholders shared with the PHP catalog.
		var usedText = tr('seatsUsedText', '{used} of {total} seats used')
			.replace('{used}', String(assigned))
	...
	function buildSeatRow(seat) {
		...
		tr2.className = 'dc-license-seat-row';
		tr2.setAttribute('data-uid', seat.uid);

		var personTd = document.createElement('td');
		personTd.className = 'dc-license-seat-row__person';
		var nameSpan = document.createElement('span');
		nameSpan.className = 'dc-license-seat-row__name';
		...
		if (seat.displayName && seat.displayName !== seat.uid) {
			...
			uidSpan.className = 'dc-license-seat-row__uid';
		...
		if (seat.withinLimit === false) {
			...
			overBadge.className = 'dc-license-badge dc-license-badge--warning dc-license-seat-row__over';
		...
		assignedTd.className = 'dc-license-seat-row__assigned';
		assignedTd.textContent = formatUnixDate(seat.assignedAt);
		tr2.appendChild(assignedTd);

		var actionsTd = document.createElement('td');
		actionsTd.className = 'dc-license-seat-row__actions';
		var removeSeatBtn = document.createElement('button');
		removeSeatBtn.type = 'button';
		removeSeatBtn.className = 'button dc-license-seat-remove';
		removeSeatBtn.setAttribute('data-uid', seat.uid);
		removeSeatBtn.setAttribute(
			'aria-label',
			tr('seatRemoveAria', 'Remove seat for {name}').replace('{name}', seat.displayName || seat.uid)
		);
	...
	function renderSeatsTable(rows) {
		...
		if (!rows || rows.length === 0) {
			...
			emptyRow.className = 'dc-license-seat-row dc-license-seat-row--empty';
			emptyRow.id = 'dc-license-seat-empty-row';
	...
	function setBusy(button, busy, busyLabel) {
		...
		if (busy) {
			button.setAttribute('data-dc-original-label', button.textContent);
			button.textContent = busyLabel;
		} else {
			var original = button.getAttribute('data-dc-original-label');
			if (original) {
	...
	if (applyForm) {
		applyForm.addEventListener('submit', function (e) {
			...
			if (!key) {
				showFeedback(tr('keyRequired', 'Paste a DTY2 license key.'), 'error');
				if (keyInput) {
					keyInput.focus();
				}
				return;
			...
			request(api.license, {
				...
				.then(function (res) {
					...
					if (!res.ok) {
						if (res.status === 0) {
							showNetworkErrorUnlessAbort(res);
							return;
						}
				...
				.catch(function (cause) {
					setBusy(saveBtn, false);
					showNetworkErrorUnlessAbort(cause);
				});
		});
	}
	...
	if (modalEl) {
		modalEl.addEventListener('click', function (e) {
			if (e.target && e.target.getAttribute && e.target.getAttribute('data-dc-license-modal-dismiss') === '1') {
				closeModal();
			}
		});
	}
	...
	function renderSuggestions(items, errMessage) {
		...
		if (errMessage) {
			...
			errP.className = 'dc-license-seat-search__note dc-license-seat-search__note--err';
		...
		if (filtered.length === 0) {
			...
			noneP.className = 'dc-license-seat-search__note';
		...
		listbox.className = 'dc-license-seat-search__listbox';
		listbox.setAttribute('role', 'listbox');
		listbox.id = 'dc-license-seat-search-listbox';
		filtered.forEach(function (item, idx) {
			var li = document.createElement('li');
			li.setAttribute('role', 'option');
			li.id = 'dc-license-seat-search-option-' + idx;
			li.className = 'dc-license-seat-search__option';

			var nameLine = document.createElement('div');
			nameLine.className = 'dc-license-seat-search__option-name';
			nameLine.textContent = item.displayName || item.id;
			li.appendChild(nameLine);
			if (item.displayName && item.displayName !== item.id) {
				var idLine = document.createElement('div');
				idLine.className = 'dc-license-seat-search__option-id';
		...
		seatSearchInput.addEventListener('input', function () {
			...
			searchDebounce = window.setTimeout(function () {
				...
				request(url.toString(), { method: 'GET' }).then(function (res) {
					...
					if (res.aborted || isNavOrAbort(res)) {
						return;
					}
					if (res.status === 401) {
