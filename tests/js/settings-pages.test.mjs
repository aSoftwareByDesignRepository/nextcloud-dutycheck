...
function loadModule() {
	const fakeWindow = {};
	// The IIFE resolves `window` lexically; injecting it as a parameter runs
	// the untouched production source against a controlled global.
	new Function('window', source)(fakeWindow);
	const api = fakeWindow.DutyCheckSettingsLegacyRedirect;
	assert.ok(api, 'module must export window.DutyCheckSettingsLegacyRedirect');
	return api;
}

const SECTION_URLS = Object.freeze({
	access: '/apps/dutycheck/settings/access',
	'duty-roles': '/apps/dutycheck/settings/duty-roles',
	planning: '/apps/dutycheck/settings/planning',
	companies: '/apps/dutycheck/settings/companies',
	conflicts: '/apps/dutycheck/settings/conflicts',
	'shift-templates': '/apps/dutycheck/settings/shift-templates',
	...
	support: '/apps/dutycheck/settings/support',
});
...
function docStub({ section = 'access', urls = { settingsSections: SECTION_URLS }, attrs = null } = {}) {
	const attributes = attrs || {
		'data-dc-settings-section': section,
		'data-dc-urls': JSON.stringify(urls),
	};
	return {
		getElementById(id) {
			if (id !== 'app-content') {
...
test('exports a frozen api with a frozen anchor map', () => {
	...
	assert.throws(() => {
		'use strict';
		api.ANCHOR_SECTIONS['dc-evil'] = 'access';
	}, TypeError);
...
test('every anchor forwards to its owning sub-page and keeps the fragment', () => {
	...
	assert.ok(anchors.length >= 14, 'anchor map must cover the full legacy page');
	for (const [anchor, section] of anchors) {
		// Simulate landing on a *different* section than the anchor's owner.
		const current = section === 'access' ? 'privacy' : 'access';
...
test('does nothing outside a settings sub-page (empty section attribute)', () => {
	const api = loadModule();
	assert.equal(api.resolve(docStub({ section: '' }), { hash: '#dc-settings-quals' }), null);
});

test('fails closed on missing DOM, malformed urls, and missing section urls', () => {
	const api = loadModule();
	const hash = '#dc-settings-quals';
	...
	assert.equal(
		api.resolve(docStub({ attrs: { 'data-dc-settings-section': 'access', 'data-dc-urls': '{broken json' } }), { hash }),
		null,
		'malformed data-dc-urls JSON',
	);
	assert.equal(
	...
	assert.equal(
		api.resolve(docStub({ urls: { settingsSections: { qualifications: '' } } }), { hash }),
		null,
		'empty target url',
	);
	assert.equal(
		api.resolve(docStub({ urls: { settingsSections: { qualifications: 42 } } }), { hash }),
		null,
