...
describe('dutycheck keep-focused-visible', () => {
	beforeEach(() => {
		_resetPadHostForTests();
	...
	it('desktop focus without soft keyboard never scrolls or pads', () => {
		...
		text.matches = () => true;
		text.closest = (sel) =>
			String(sel).includes('.dc-dialog') || String(sel).includes('[role="dialog"]') || String(sel).includes('.modal')
				? host
				: null;
