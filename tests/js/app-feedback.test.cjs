#!/usr/bin/env node
/**
 * Behavioural tests for DutyCheck app-feedback.js (mailto + toast hook).
 *
...
function boot(extra) {
	...
	const config = {
		appId: 'dutycheck',
		appDisplayName: 'DutyCheck',
		appVersion: '1.0.0',
		feedbackEmail: 'dev@software-by-design.de',
		githubIssuesUrl: 'https://github.com/aSoftwareByDesignRepository/nextcloud-dutycheck/issues',
		cssPrefix: 'dc',
		...(extra || {}),
	};
	const cfgEl = {
	...
	nodes.set('dc-app-feedback-config', cfgEl);
	...
	nodes.set('dc-feedback-problem', problem);
	nodes.set('dc-feedback-idea', idea);

	const trigger = {
		className: 'dc-nav-footer__trigger',
		getAttribute(k) {
			return this[k] || null;
		},
		setAttribute(k, v) {
	...
	trigger['aria-controls'] = 'dc-feedback-menu';
	const menu = {
		id: 'dc-feedback-menu',
		hidden: true,
		contains() {
	...
	nodes.set('dc-feedback-menu', menu);
	doc.querySelector = (sel) => {
		if (sel === '.dc-nav-footer__trigger') {
			return trigger;
		}
		return null;
	};
	doc.getElementById = (id) => {
		if (id === 'dc-feedback-menu') {
			return menu;
		}
		return nodes.get(id) || null;
	};
	...
	const sandbox = {
		...
		location: {
			...
			pathname: '/apps/dutycheck/today',
			search: '?token=secret&keep=1',
			href: 'https://cloud.example/apps/dutycheck/today?token=secret&keep=1',
		},
...
function main() {
	...
	const clean = sandbox.SbdAppFeedback.sanitizePageUrl(
		'https://cloud.example/apps/dutycheck/x?token=abc&keep=1&password=no'
	);
	...
	const upgradeAck = sandbox.SbdAppFeedback.sanitizePageUrl(
		'/apps/dutycheck/?IKnowThatThisIsABigInstanceAndTheUpdateRequestCouldRunIntoATimeoutAndHowToRestoreABackup=IAmSuperSureToDoThis'
	);
	assert(upgradeAck === '/apps/dutycheck/', 'strips Nextcloud web-upgrade ack query key');
	...
	process.stdout.write('app-feedback.test.js OK (dutycheck)\n');
}
