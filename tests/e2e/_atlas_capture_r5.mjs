 ...
 * Atlas visual-fix r5 — dark roster (new hashes), loaded Today (no skeleton),
...
async function forceDarkDom(page) {
	await page.evaluate(() => {
		const apply = () => {
			...
			const styleId = 'dc-atlas-r5-dark-force'
...
async function shot(page, name) {
	...
	await settle(page)
	const qaPath = join(outQa, `atlas-visual-r5-${name}.png`)
	const atlasPath = join(outAtlas, `atlas-visual-r5-${name}.png`)
...
await dismissTips(page)
// Signature craft: replace loading pulse with coverage chips (DE).
await page.evaluate(() => {
...
const qaGrid = join(outQa, 'atlas-visual-r5-web-roster-month-grid.png')
const atlasGrid = join(outAtlas, 'atlas-visual-r5-web-roster-month-grid.png')
...
writeFileSync(join(outAtlas, 'atlas-visual-r5-manifest.json'), JSON.stringify(manifest, null, 2))
console.log('r5 web screenshots →', outAtlas)
console.log(JSON.stringify(manifest))
} catch (err) {
	console.error('r5 capture failed', err?.message || err)
