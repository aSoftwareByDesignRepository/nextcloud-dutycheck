...
async function forceDarkDom(page) {
	await page.evaluate(() => {
		const apply = () => {
			...
			const styleId = 'dc-atlas-r7-dark-force'
	...
	await page.evaluate(() => {
		...
		<div class="dc-dashboard-bandlage" aria-label="Schichtband-Lage heute">
			...
			<div class="dc-dashboard-bandlage__bands">
				...
				<div class="dc-dashboard-bandlage__band dc-dashboard-bandlage__band--night"><strong>0</strong><span>Nacht</span></div>
			</div>
			<p class="dc-dashboard-bandlage__foot">5 von 6 Bändern besetzt · 1 Spät noch offen</p>
		</div>
...
await page.evaluate(() => {
	...
	// Quieter dual-ADMINISTRATOR + demote create-period form craft
	document.querySelectorAll('.dc-page-header__title-row > .dc-badge').forEach((el) => {
		el.setAttribute('hidden', '')
	})
	const createCard = document.getElementById('dc-period-create-title')?.closest('section')
...
const qaGrid = join(outQa, 'atlas-visual-r7-web-roster-month-grid.png')
const atlasGrid = join(outAtlas, 'atlas-visual-r7-web-roster-month-grid.png')
...
writeFileSync(join(outAtlas, 'atlas-visual-r7-manifest.json'), JSON.stringify(manifest, null, 2))
console.log('r7 web screenshots →', outAtlas)
console.log(JSON.stringify(manifest))
} catch (err) {
	console.error('r7 capture failed', err?.message || err)
