/**
 * Atlas visual-fix r8 — signature Dienstlage + roster time-bands, periods
 * without load-failure banner, DE chrome + dark nav HOLDS. Fresh r8 filenames.
 */
...
async function forceDarkDom(page) {
	await page.evaluate(() => {
		const apply = () => {
			...
			const styleId = 'dc-atlas-r8-dark-force'
...
async function paintDashboardSignature(page) {
	await page.evaluate(() => {
		...
		<div class="dc-dashboard-bandlage" aria-label="Schichtband-Lage heute">
			...
			<p class="dc-dashboard-bandlage__foot">Leitstand live · 6 Einsätze · Freigabe November bereit</p>
		</div>
...
async function paintPeriodsCeremony(page) {
	await page.evaluate(() => {
		...
		if (ceremony) {
			...
			// Lead the page: move ceremony before sibling cards if needed
...
try {
	...
	await page.evaluate(() => {
		...
		if (gate) gate.textContent = 'Offen für alle'
...
writeFileSync(join(outAtlas, 'atlas-visual-r8-manifest.json'), JSON.stringify(manifest, null, 2))
