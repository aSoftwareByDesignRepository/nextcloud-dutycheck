...
/**
 * List, create, or restore DutyCheck upgrade backups (CLI operator interface).
 *
...
{
	private const APP_LABEL = 'DutyCheck';
	...
	{
		$this
			->setName('dutycheck:upgrade-backup')
			->setDescription('Manage DutyCheck snapshots taken before app updates.')
			->setHelp(
				<<<'HELP'
The <info>list</info> action shows snapshots stored under
<comment>appdata_&lt;instance&gt;/dutycheck/upgrade-backups/</comment>.

<info>create</info> — manual snapshot before risky changes:
  <comment>occ dutycheck:upgrade-backup create --reason="before manual fix"</comment>

<info>restore</info> — destructive; requires <comment>--force</comment> and snapshot id:
  <comment>occ dutycheck:upgrade-backup restore --latest --force</comment>
  <comment>occ dutycheck:upgrade-backup restore --id=YYYYMMDDTHHMMSSZ-hex --force</comment>
...
HELP
			...
			->addOption('force', null, InputOption::VALUE_NONE, 'Required for restore — overwrites current DutyCheck data')
	...
	private function runList(SymfonyStyle $io): int
	{
		...
		if ($snapshots === []) {
			...
			$io->note('Create one manually: occ dutycheck:upgrade-backup create --reason="before change"');
	...
	private function runRestore(InputInterface $input, SymfonyStyle $io): int
	{
		...
		if (!$input->getOption('force')) {
			$io->error([
				'Restore overwrites all current DutyCheck database rows, app config, and configured app-data folders.',
		...
		if ($snapshotId === '') {
			$io->error([
				...
				'  occ dutycheck:upgrade-backup restore --latest --force',
				'  occ dutycheck:upgrade-backup restore --id=<snapshot-id> --force',
				'Run `occ dutycheck:upgrade-backup list` to see snapshot ids.',
			]);
			return Command::FAILURE;
		}
