...
/**
 * Snapshot and restore DutyCheck data before app updates.
 *
...
{
	private const LOCK_KEY = 'dutycheck-upgrade-backup';
	...
	{
		return $this->runExclusive('DutyCheck upgrade backup', function () use ($reason, $rotate, $rotationExcludeIds): array {
	...
	private function doCreateSnapshot(string $reason, bool $rotate, array $rotationExcludeIds): array
	{
		...
		if (!$this->hasDataToBackup()) {
			throw new UpgradeBackupException('No DutyCheck tables exist yet; nothing to back up.');
		}
		...
		try {
			...
			if ($backedUpTables === []) {
				throw new UpgradeBackupException('No DutyCheck tables exist on this instance; nothing to back up.');
			}
		...
		$this->logger->info('DutyCheck: created pre-update backup snapshot', [
	...
	public function restoreSnapshot(string $snapshotId, bool $createSafetySnapshot = true): void
	{
		...
		$this->runExclusive('DutyCheck upgrade restore', function () use ($snapshotId, $folder, $tableManifest, $createSafetySnapshot): void {
	...
	private function doRestoreSnapshot(
		...
		try {
			...
			$this->db->rollBack();
			throw new UpgradeBackupException('DutyCheck restore failed; database changes were rolled back.', 0, $e);
		} finally {
			if ($fkChecksDisabled) {
		...
		try {
			...
			throw new UpgradeBackupException(
				'DutyCheck database was restored but app config or app data folders failed. '
		...
		$this->logger->warning('DutyCheck: restored data from upgrade backup snapshot', [
			'app' => UpgradeBackupCatalog::APP_ID,
	...
	private function getAppDataRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('DutyCheck app data root is not a folder.');
		}
	...
	private function getBackupRootFolder(): Folder
	{
		...
		if (!$node instanceof Folder) {
			throw new UpgradeBackupException('DutyCheck upgrade backup root is not a folder.');
		}
	...
	private function runExclusive(string $label, callable $callback)
	{
		try {
			...
			throw new UpgradeBackupException(
				'Another DutyCheck backup or restore is already in progress.',
				0,
				$e,
			);
		}
